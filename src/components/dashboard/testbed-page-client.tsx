...
import { AlertTriangle, Clock3, Download, Play, Shield, Sparkles } from 'lucide-react';
...
function EvalPdfDocument({ events }: { events: EventEntry[] }) {
  return (
    <Document>
      <Page size="A4" style={pdfStyles.page}>
        <Text style={pdfStyles.title}>Ægis Evaluation Report</Text>
        <Text>Generated on {new Date().toLocaleString()}</Text>
        <Text>Total Events: {events.length}</Text>
        <View style={pdfStyles.table}>
          <View style={pdfStyles.tableRow}>
            <View style={pdfStyles.tableCol}>
              <Text style={pdfStyles.tableCell}>Time</Text>
            </View>
            <View style={pdfStyles.tableCol}>
              <Text style={pdfStyles.tableCell}>Attack</Text>
            </View>
            <View style={pdfStyles.tableCol}>
              <Text style={pdfStyles.tableCell}>Provider</Text>
            </View>
            <View style={pdfStyles.tableCol}>
              <Text style={pdfStyles.tableCell}>Status</Text>
            </View>
            <View style={pdfStyles.tableCol}>
              <Text style={pdfStyles.tableCell}>Safety Score</Text>
            </View>
          </View>
          {events.map((event) => (
            <View key={event.id} style={pdfStyles.tableRow}>
              <View style={pdfStyles.tableCol}>
                <Text style={pdfStyles.tableCell}>{formatTimestamp(event.createdAt)}</Text>
              </View>
              <View style={pdfStyles.tableCol}>
                <Text style={pdfStyles.tableCell}>{event.attackTitle}</Text>
              </View>
              <View style={pdfStyles.tableCol}>
                <Text style={pdfStyles.tableCell}>{event.provider}</Text>
              </View>
              <View style={pdfStyles.tableCol}>
                <Text style={pdfStyles.tableCell}>{event.status}</Text>
              </View>
              <View style={pdfStyles.tableCol}>
                <Text style={pdfStyles.tableCell}>{formatSafetyScore(event.safetyScore)}</Text>
              </View>
            </View>
          ))}
        </View>
...
type TestbedPageClientProps = {
  feedbackEnabled: boolean;
};

export default function TestbedPageClient({
  feedbackEnabled,
}: TestbedPageClientProps) {
  ...
  return (
    <section className="space-y-6 pb-20">
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Chronological request history with hardening output.
              </CardDescription>
            </div>
            {eventLog.length > 0 ? (
              <PDFDownloadLink
                document={<EvalPdfDocument events={eventLog} />}
                fileName="aegis-eval-report.pdf"
              >
                {({ loading }) => (
      ...
      <FeedbackWidget enabled={feedbackEnabled} />
    </section>
  );
}
